...
class AuditingConnectorSpec
  ...
  "For auditing message Type event" - {
    "when sending an audit message" - Seq(MimeTypes.XML, MimeTypes.JSON).foreach { contentType =>
      s"when the content-type equals $contentType" - {
        ...
        "return a failed future if the audit message was not accepted" - Seq(BAD_REQUEST, INTERNAL_SERVER_ERROR).foreach { statusCode =>
          s"when a $statusCode is returned" in forAll(
            ...
            val result = future
              .map(_ => fail("A success was registered when it should have been a failure."))
              .recover {
