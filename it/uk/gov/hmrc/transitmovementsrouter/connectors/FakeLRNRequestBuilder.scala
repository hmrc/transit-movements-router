...
import uk.gov.hmrc.transitmovementsrouter.models.errors.ErrorCode.Forbidden
...
class FakeLRNRequestBuilder extends RequestBuilder {
  override def transform(transform: WSRequest => WSRequest): RequestBuilder = this

  override def execute[A](implicit evidence$1: HttpReads[A], ec: ExecutionContext): Future[A] =
    Future.failed(UpstreamErrorResponse("The supplied LRN: DUPLRN1234 has already been used by submitter: test", 403))

  override def stream[A](implicit evidence$2: StreamHttpReads[A], ec: ExecutionContext): Future[A] =
    Future.failed(UpstreamErrorResponse("The supplied LRN: DUPLRN1234 has already been used by submitter: test", 403))
