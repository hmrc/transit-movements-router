...
class RoutingServiceImpl @Inject() (messageConnectorProvider: EISConnectorProvider)(implicit materializer: Materializer)
  ...
  private def postToEis(
    ...
    office match {
      case _ if office.isGB => messageConnectorProvider.gb.post(messageSender, body, hc)
      case _ if office.isXI => messageConnectorProvider.xi.post(messageSender, body, hc)
      case _                => Future.successful(Left(RoutingError.UnrecognisedOffice(s"Did not recognise office: ${office.value}")))
    }
}
